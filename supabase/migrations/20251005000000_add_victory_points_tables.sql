-- Victory Points System - Additional tables
-- Note: playtest_reviews already exists in main schema

-- User Victory Points balance
CREATE TABLE IF NOT EXISTS user_victory_points (
...
CREATE TABLE IF NOT EXISTS victory_points_transactions (
...
-- Indexes
CREATE INDEX IF NOT EXISTS victory_points_transactions_user_id_idx ON victory_points_transactions(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS victory_points_transactions_type_idx ON victory_points_transactions(transaction_type, created_at DESC);
...
BEGIN
  ...
  IF TG_OP != 'DELETE' AND review_author_id = NEW.voter_id THEN
...
DROP TRIGGER IF EXISTS award_review_vp_trigger ON playtest_reviews;
