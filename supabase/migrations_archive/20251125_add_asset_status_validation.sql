...
-- This replaces the previous 3-state system (draft, public, archived)
...
-- Step 2: Migrate existing 'public' assets to 'private' (safer default - keeps assets exclusive to owner)
...
-- Drop old policy that checked for 'public' status
DROP POLICY IF EXISTS "public assets are viewable by everyone" ON public.assets;
...
DROP POLICY IF EXISTS "Asset files are viewable for public assets" ON public.asset_files;
...
DROP POLICY IF EXISTS "Asset images are viewable for public assets" ON public.asset_images;
