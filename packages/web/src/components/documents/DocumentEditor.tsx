...
import { subscribeToDocumentContent } from '@gameloopers/core/realtime/document-content-subscription';
...
export default function DocumentEditor(props: DocumentEditorProps) {
  ...
  let broadcastContent:
    ((content: Record<string, unknown>, userId: string) => void) | null = null;
  let updatePresence:
    | ((presence: {
        userId: string;
        userName: string | null;
  ...
  return (
    <div
      ...
      classList={{ 'document-editor--fullscreen': isFullscreen() }}
    >
      {props.canEdit && (
        <div class="document-editor__toolbar">
            ...
            >
              {isUploadingImage() ? (
                <svg
                  ...
                  stroke="currentColor"
                  stroke-width="2"
                  class="document-editor__spinner"
                >
          ...
          {activeEditors().length > 0 && (
            <div class="document-editor__presence">
              <For each={activeEditors()}>
                {(editor) => (
                  <div
                    class="document-editor__presence-avatar"
                    title={editor.userName || editor.userHandle || 'Unknown'}
                  >
                    {(editor.userName || editor.userHandle || '?')
                      .charAt(0)
                      .toUpperCase()}
                  </div>
                )}
          ...
          <div class="document-editor__save-status">
            ...
            {!isSaving() && !hasUnsavedChanges() && lastSaved() && (
              <span class="document-editor__saved">
