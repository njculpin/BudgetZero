...
} from '../content-item/content-item';

/**
 * The fields this modal reads from `/api/products/search-embeddable`.
 *
 * Deliberately narrow: a component should state what it needs rather than
 * restate the whole product model, which would go stale the moment the endpoint
 * returns something different.
 */
interface EmbeddableSearchResult {
  id: string;
  title: string;
  creator_name: string;
...
/** The fields this modal reads from `/api/documents/user-documents`. */
            ...
            <Show when={searchResults().length > 0}>
              <div class="add-content-modal__search-results">
                <For each={searchResults()}>
                  {(product: EmbeddableSearchResult) => (
