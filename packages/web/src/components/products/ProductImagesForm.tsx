        ...
        >
          ...
          <For each={images()}>
            {(image, index) => (
              ...
              >
                <img src={image.file_url} alt={image.title} class="image-grid__image" />
                {index() === 0 && <span class="image-grid__badge">Cover</span>}
          ...
          <For each={uploadQueue()}>
            {(upload) => (
              <div class="image-grid__item image-grid__item--uploading">
                <img src={upload.preview} alt="Uploading..." class="image-grid__image" />
      ...
      <Show when={availableSlots() > 0 && uploadQueue().length === 0}>
        <label class="image-grid-editor__upload-btn">
          ...
          <span>
            {images().length === 0
              ? 'Add Images'
              : `Add More Images (${availableSlots()} remaining)`}
          </span>
        </label>
      </Show>

      <Show when={availableSlots() === 0}>
        <p class="image-grid-editor__limit">Maximum of {MAX_IMAGES} images reached</p>
      </Show>
    </div>
  );
}
