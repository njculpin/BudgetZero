...
import { cartRemove } from '@gameloopers/api/controllers/cart/remove';
...
describe('POST /api/cart/remove', () => {
  ...
  describe('Authentication', () => {
    ...
    it('rejects a caller who is not signed in', async () => {
      const response = await cartRemove(makeContext({ userId: null, body: {} }));
  ...
  describe('Cart Item Removal', () => {
    it('should remove cart item successfully', async () => {
      ...
      const response = await cartRemove(makeContext({ request: mockRequest }));
    ...
    it('should call removeFromCart with correct cart item ID', async () => {
      ...
      await cartRemove(makeContext({ request: mockRequest }));
