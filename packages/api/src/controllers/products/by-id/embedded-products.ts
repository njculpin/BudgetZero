...
/** The joined `products -> users` row this endpoint selects, with to-one embeds resolved. */
interface EmbeddedProductRow {
  ...
  status: string;
  user_id: string;
  users: { name: string | null; handle: string } | null;
}
...
export const productsProductIdEmbeddedProducts: Controller = async ({ params }) => {
  ...
  try {
    ...
    if (!components || components.length === 0) {
      return new Response(JSON.stringify({ embeddedProducts: [] }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    const embeddedProducts = components.map((component) => {
      // Both embeds follow a foreign key to a single row, so PostgREST returns
      // an object. The generated types say array because cardinality is not
      // recoverable from the select string.
      const product = component.products as unknown as EmbeddedProductRow;
      const creator = product.users;
    ...
    return new Response(
      JSON.stringify({
        error:
          error instanceof Error ? error.message : 'Failed to fetch embedded products',
      }),
      {
        status: 500,
        headers: { 'Content-Type': 'application/json' },
