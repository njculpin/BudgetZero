...
import { authCallback } from '@gameloopers/api/controllers/auth/callback';
    ...
    it('should exchange code for session and set cookies', async () => {
      ...
      expect(mockCookies.set).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          secure: false,
          sameSite: 'lax',
          maxAge: 60 * 60 * 24 * 30, // 30 days - outlives the access cookie
        })
      );
    ...
    it('should set cookies with correct security attributes', async () => {
      ...
      expect(mockCookies.set).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          path: '/',
          sameSite: 'lax',
          maxAge: 2592000, // 30 days - the refresh cookie outlives the access cookie
        })
      );
    });
