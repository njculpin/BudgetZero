...
export class GameProjectService {
  ...
  async createProject(
    userId: string,
    projectData: Omit<CreateGameProjectData, "creator_id" | "slug">,
  ): Promise<ApiResponse<GameProject>> {
    try {
      // Validate input data
  ...
  async getProjectWithCollaborators(
    projectId: string,
  ...
  async getUserProjects(
    ...
    page = 1,
    limit = 20,
  ): Promise<ApiResponse<PaginatedResponse<GameProjectWithCreator>>> {
    try {
      ...
      if (collaboratorProjectIds && collaboratorProjectIds.length > 0) {
        const projectIds = collaboratorProjectIds.map(
    ...
    try {
      ...
      if (filters.complexity_rating) {
        queryBuilder = queryBuilder.eq(
          "complexity_rating",
          filters.complexity_rating,
        );
      }
      if (filters.player_count_min) {
        queryBuilder = queryBuilder.gte(
          "player_count_max",
          filters.player_count_min,
        );
      }
      if (filters.player_count_max) {
        queryBuilder = queryBuilder.lte(
          "player_count_min",
          filters.player_count_max,
        );
      }
  ...
  async checkProjectAccess(
    ...
    ApiResponse<{ canRead: boolean; canEdit: boolean; canAdmin: boolean }>
  > {
    try {
